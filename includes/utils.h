 ...
 * @LastEditors: Nestcc
 * @LastEditTime: 2022-01-24 01:25:59
...
#define ALLOCATE(vm_ptr, type) (type *)mem_manager(vm_ptr, NULL, 0, sizeof(type));
...
#define DESTORY(vm_ptr, mem_ptr) mem_manager(vm_ptr, mem_ptr, 0, 0);
...
#define IO_ERROR(...) report_error(NULL, ERROR_IO, __VA_ARGS__);

#define MEM_ERROR(...) report_error(NULL, ERROR_MEM, __VA_ARGS__);

#define LEX_ERROR(parser, ...) report_error(parser, ERROR_LEX, __VA_ARGS__);

#define COMPILE_ERROR(parser, ...) report_error(parser, ERROR_COMPILE, __VA_ARGS__);
